...
class ShopItemViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addShopItem(inputName: String?, inputCount: String?, inputPrice: String?) {
        ...
        if (fieldsValidated) {
            viewModelScope.launch {
                val shopItem = ShopItem(name, count, true, price)
    ...
    fun editShopItem(inputName: String?, inputCount: String?, inputPrice: String?) {
        val price = parsePrice(inputPrice)
        val name = parseName(inputName)
        val count = parseCount(inputCount)
        val fieldsValidated = validateInput(name, count)
        if (fieldsValidated) {
            viewModelScope.launch {
                _shopItem.value?.let {
                    val item = it.copy(name = name, count = count, price = price, enabled = true)
